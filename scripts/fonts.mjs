...
await fs.writeFile('assets/css/fonts.css',local.join('\n')+'\n');
for(const family of ['lato','playfairdisplay']){
  await fs.writeFile(`assets/fonts/${family}-OFL.txt`,await fetchText(`https://raw.githubusercontent.com/google/fonts/main/ofl/${family}/OFL.txt`));
}
