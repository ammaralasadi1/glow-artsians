...
const holidayCities = JSON.parse(await readFile(new URL('content/holiday-cities.json',root),'utf8'));
const holidayServices = JSON.parse(await readFile(new URL('content/holiday-services.json',root),'utf8'));
const pages = [
  ['/', 'index.html'],
...
test('serves styles, scripts, images, and fonts with their browser MIME types', async () => {
  const assets = [
    ['/assets/css/fonts.css', /^text\/css\b/i],
    ['/assets/js/site-shell.js', /^(?:text|application)\/javascript\b/i],
...
test('unknown pages and private project files return 404', async () => {
  for (const path of ['/missing-page', '/missing-page.html', '/assets/missing.webp', '/.git/config', '/package.json', '/scripts/serve.mjs', '/partials/header.html']) {
