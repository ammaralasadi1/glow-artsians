...
export async function refreshHolidayImagery(html) {
  const inventory=JSON.parse(await fs.readFile('content/holiday-imagery.json','utf8'));
  const manifest=JSON.parse(await fs.readFile('assets/images/responsive-manifest.json','utf8'));
  ...
  const disclosure='Still imagery is AI-generated design inspiration, not photography of completed projects.';
  if(!html.includes(disclosure)){
    if(html.includes('class="city-scope-note"'))html=html.replace(/(<p class="city-scope-note">[\s\S]*?)(<\/p>)/,`$1 ${disclosure}$2`);
    else html=html.replace('</main>',`<p class="consultation-service-area">${disclosure}</p>\n  </main>`);
  }
